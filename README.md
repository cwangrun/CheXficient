...
Please refer to ./models_clip.py for downstream-adapted tasks like:

1. Classification (linear probing);
2. Segmentation (U-Net decoding);
3. Radiology report generation, we adopt the VLM framework from Microsoft’s [LLaVA-Rad](https://github.com/microsoft/LLaVA-Rad), replacing its original image encoder with our pre-trained vision encoder.
