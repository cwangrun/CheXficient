...
from engine import train_one_epoch, evaluate, warm_up_and_initialize
...
def main_worker(gpu, args):
    ...
    # checkpoint = torch.load("./to_hf/checkpoint_chexficient.pth", map_location='cpu')
    ...
    if step[0] == 0 and getattr(args, "warm_up_ratio", 0.0) > 0:

        warmup_batch_size = args.batch_size
        warmup_sampler = None
        if dist.is_initialized() and dist.get_world_size() > 1:
            warmup_sampler = torch.utils.data.distributed.DistributedSampler(
                dataset_train,
        ...
        def producer_fn(epoch):
            while True:
                if warmup_sampler is not None:
                    warmup_sampler.set_epoch(epoch)
        ...
        if dist.is_initialized() and dist.get_world_size() > 1:
            warm_up_and_initialize(
                step, gpu, producer_iter, len(warmup_data_loader_producer), model, model_without_ddp,
                optimizer, loss_scaler, args
            )
        else:
            warm_up_and_initialize(
                step, gpu, producer_iter, len(warmup_data_loader_producer), model, model_without_ddp,
